...
#rmarkdown::render(paste(rootdir,"rmd/cooperative_environment_degree2_invisible_reverse.Rmd", sep="/"))
...
if(modelForPrediction=="random forest"){
  ...
  if(V==0){
    model1<-model1(redo=TRUE,visible=FALSE)
    model2<-model2(redo=TRUE,visible=FALSE)
    model3<-model3(redo=TRUE,visible=FALSE)
    }
  if(V==1){
    model1<-model1(redo=TRUE,visible=TRUE)
    model2<-model2(redo=TRUE,visible=TRUE)
    model3<-model3(redo=TRUE,visible=TRUE)
    }
}
  ...
  fractionCoop = frac
    ...
    for(m in c(1:iterations)){
      ...
      for (k in 1:L)
      {
        ...
        if(modelForPrediction=="random forest"){
          if (k==1) {
              if(V==1){node_rX$prob_coop = predict(model1,node_rX[c("coop_rp1","coop_rp2","GINI")])}
              else if(V==0){node_rX$prob_coop = predict(model1,node_rX[c("coop_rp1","coop_rp2")])}
            } else {
              if(V==1){node_rX$prob_coop = predict(model2,node_rX[c("prev_coop","GINI","prev_avg_env_wealth","prev_wealth")])}
              else if(V==0){node_rX$prob_coop = predict(model2,node_rX[c("prev_coop","prev_avg_env_wealth","prev_wealth")])}
            }
        }
