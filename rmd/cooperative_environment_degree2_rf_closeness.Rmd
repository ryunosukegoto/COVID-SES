---
title: "Altering social network position prevents social isolation (using closeness centrality)"
...
#rmarkdown::render(paste(rootdir,"rmd/cooperative_environment_degree2_rf_closeness.Rmd", sep="/"))
  ...
  for(V in c(0,1)){
      ...
      for(frac in c(0,0.25,0.5,0.75,1)){
        ...
        fractionCoop = frac
          ...
          for(m in c(1:iterations)){
            ...
            node_r0$prev_closeness = closeness(g_r0)
            ...
            for (k in 1:L)
            {
              ...
              node_rX$round = node_rX$round + 1
              node_rX[is.na(node_rX$prev_degree)==1,"prev_degree"] = 0
              node_rX[is.na(node_rX$prev_closeness)==1,"prev_closeness"] = 0
              node_rX[is.na(node_rX$prev_local_rate_coop)==1,"prev_local_rate_coop"] = 0
              #Only this calculation needs to change from Round 1
              ...
              if(k==1){
                ...
                if(fractionCoop>0){
                  prob_coop_df = NULL
                  nodesCoop = NULL
                  #nodesCoop = node_rX$prev_degree<=quantile(node_rX$prev_degree,fractionCoop) #assign low-closeness centrality nodes to cooperators
                  #assign defectors to designated nodes
                  
                  if(fractionCoop<1){nodesCoop = node_rX$prev_closeness<quantile(node_rX$prev_closeness,fractionCoop) & node_rX$prev_closeness>=quantile(node_rX$prev_closeness,fractionCoop-0.25)}
                  else if(fractionCoop==1){nodesCoop = node_rX$prev_closeness<=quantile(node_rX$prev_closeness,fractionCoop) & node_rX$prev_closeness>=quantile(node_rX$prev_closeness,fractionCoop-0.25)}
                  prob_coop_df = 
                    data.frame(
              ...
              node_import$prev_closeness = closeness(g_rX)
    ...
    g.percentIsolation = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="percentIsolation", add = "mean_se", color="coopFrac") +
      ...
      labs(
        ...
        x = "Closeness centrality percentile of nodes assigned to defectors ",
...
for(i in unique(plot.trends$fractionCoop)){
  ...
  print(annotate_figure(plot, top = text_grob(paste("Closeness centrality percentile of nodes assigned to defectors =",i), color = "black", face = "bold", size = 10)))
}
