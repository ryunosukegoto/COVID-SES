...
#rmarkdown::render(paste(rootdir,"rmd/cooperative_environment_degree2_rf_models_no_egobehavior.Rmd", sep="/"))
...
model3 <- model3_no_egobehavior(redo=TRUE)
...
for(A in c(1150,700,500)){
  for(V in c(0,1)){
    ...
    g.percentIsolation = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="percentIsolation", add = "mean_se") +
      stat_compare_means(ref.group = "0.25", label = "p.signif", label.y = 0.198, method="t.test", color="black") +  
      labs(
      ...
      annotate("text", x=1, y=0.1990, label= "ref", color="black") +
      annotate("text", x=1.1, y= -0.0042, label= "Lowest degree nodes assigned to defectors", size=2.5) +
      geom_segment(aes(x = 1.8, y = -0.0044, xend = 3.2, yend = -0.0044), linewidth=0.2, arrow = arrow(length = unit(0.1, "cm"))) +
      annotate("text", x=3.9, y= -0.0042, label= "Highest degree nodes assigned to defectors", size=2.5) +
      theme_bw() +
