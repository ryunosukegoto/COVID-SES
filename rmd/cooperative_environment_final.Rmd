...
#rmarkdown::render(paste(rootdir,"rmd/cooperative_environment_final.Rmd", sep="/"))
...
#define isolation
sample.wide$isolated = ifelse(sample.wide$degree.1==0 | sample.wide$degree.2==0 | sample.wide$degree.3==0 | sample.wide$degree.4==0 | sample.wide$degree.5==0| sample.wide$degree.6==0 | sample.wide$degree.7==0 | sample.wide$degree.8==0 | sample.wide$degree.9==0 | sample.wide$degree.10==0, 1, 0)
```
...
for(A in c(500)){ #wealth 500 for everyone 
  for(V in c(0)){ #wealth invisible
      ...
      if(modelForPrediction=="random forest"){
        ...
        if(V==0){
          model1<-model1.invisible(redo=TRUE)
          model2<-model2.invisible(redo=TRUE)
          model3<-model3(redo=TRUE)
          }
        if(V==1){
          model1<-model1.visible(redo=TRUE)
          model2<-model2.visible(redo=TRUE)
          model3<-model3(redo=TRUE)
          }
      }
        ...
        fractionCoop = frac
          ...
          for(m in c(1:iterations)){
            ...
            for (k in 1:L)
            {
              ...
              if(modelForPrediction=="random forest"){
                if (k==1) {
                    if(V==1){node_rX$prob_coop = predict(model1[[1]],
                                                         newdata=
                                                           data.frame(
                    ...
                    else if(V==0){node_rX$prob_coop = predict(model1[[1]],
                                                         newdata=
                                                           data.frame(
                                                             behavior.p1 = coop_rp1,
                                                             behavior.p2 = coop_rp2
                    ...
                    if(V==1){node_rX$prob_coop = predict(model2[[1]],
                                                         newdata=
                                                           data.frame(
                    ...
                    else if(V==0){node_rX$prob_coop = predict(model2[[1]],
                                                         newdata=
                                                           data.frame(
                                                             prevCoop = node_rX$prev_coop,
                                                             egoPrevWealth = node_rX$prev_wealth
              ...
              if(modelForPrediction=="random forest"){
                link_rX_3$ego_prob = predict(model3[[1]],
                                             newdata=
                                               data.frame(
                ...
                link_rX_3$alt_prob = predict(model3[[1]],
                                             newdata=
                                               data.frame(
                                                 previouslyconnected = link_rX_3$prev_connected,
    ...
    g.percentIsolation = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="percentIsolation", add = "mean_se", color="coopFrac") +
      stat_compare_means(ref.group = "0", label = "p.format", label.y = 0.09998, method="t.test", color="black") +  
      labs(
      ...
      annotate("text", x=1, y=0.1001, label= "ref", color="black") +
      theme_bw() +
      ...
      scale_color_manual(values = c('0' = "black",'0.25'="black",'1'="black")) +
    ...
    g.isolationPersonRounds = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="isolationPersonRounds", add = "mean_se", color="coopFrac") +
      stat_compare_means(ref.group = "0", label = "p.format", label.y = 9.98, method="t.test", color="black") +  
      labs(
        #title = paste("Isolation when defectors are assigned to 25% of nodes by degree, ","V=",V,", Gini=",GINI,sep=""),
        x = "Treatment arm",
        y = "Person-rounds of isolation") +
      annotate("text", x=1, y=10.05, label= "ref", color="black") +
      theme_bw() +
      theme(plot.title = element_text(hjust = 0.5, size=12),legend.position="none") +
    ...
    g.percentIsolationC = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="percentIsolationC", add = "mean_se") +
      stat_compare_means(ref.group = "0", label = "p.format", label.y = 0.098, method="t.test", color="black") +  
      labs(
        #title = paste("Isolation among initial cooperators, ","V=",V,", Gini=",GINI,sep=""),
        x = "Treatment arm",
        y = "Propoption of ever-isolated cooperators") +
    ...
    g.percentIsolationD = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="percentIsolationD", add = "mean_se") +
      stat_compare_means(ref.group = "0", label = "p.format", label.y = 0.398, method="t.test", color="black") +  
      labs(
        #title = paste("Isolation among initial defectors, ","V=",V,", Gini=",GINI,sep=""),
        x = "Treatment arm",
        y = "Propoption of ever-isolated defectors") +
      annotate("text", x=1, y=0.3990, label= "ref", color="black") +
      theme_bw() +
      theme(plot.title = element_text(hjust = 0.5, size=12),legend.position="none") +
    ...
    g.avgCoopFinal = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="avgCoopFinal", add = "mean_se") +
      stat_compare_means(ref.group = "0", label = "p.format", label.y = 0.98, method="t.test", color="black") +  
      labs(
    ...
    g.avg_wealth = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="avg_wealth", add = "mean_se") +
      stat_compare_means(ref.group = "0", label = "p.format", label.y = 6800, method="t.test", color="black") +  
      labs(
    ...
    g.gini = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="gini", add = "mean_se") +
      stat_compare_means(ref.group = "0", label = "p.format", label.y = 0.48, method="t.test", color="black") +  
      labs(
    ...
    g.degree = ggbarplot(data=df.netIntLowDegree, x="coopFrac", y="degree", add = "mean_se") +
      stat_compare_means(ref.group = "0", label = "p.format", label.y = 14.8, method="t.test", color="black") +  
      labs(
...
plot.trends <- plot.trends %>%
  mutate(Treatment =
           case_when(
             fractionCoop == 0 ~ "Control",
             fractionCoop == 0.25 ~ "Low degree defectors",
             fractionCoop == 1 ~ "High degree defectors"
           ))

g.gini = ggplot(data=plot.trends, aes(x=round,y=gini_mean,group=Treatment)) +
  geom_line(aes(color=Treatment)) +
  geom_ribbon(aes(ymin = gini_mean - gini_sd, ymax = gini_mean + gini_sd, fill=Treatment),alpha=0.3) +
  xlab("Round")+
  ylab("Gini coefficient") +
  theme_bw() 

g.gmd = ggplot(data=plot.trends, aes(x=round,y=gmd_mean,group=Treatment)) +
  geom_line(aes(color=Treatment)) +
  geom_ribbon(aes(ymin = gmd_mean - gmd_sd, ymax = gmd_mean + gmd_sd, fill=Treatment),alpha=0.3) +
  xlab("Round")+
  ylab("Mean difference") +
  theme_bw() 

g.avg_wealth = ggplot(data=plot.trends, aes(x=round,y=avg_wealth_mean,group=Treatment)) +
  geom_line(aes(color=Treatment)) +
  geom_ribbon(aes(ymin = avg_wealth_mean - avg_wealth_sd, ymax = avg_wealth_mean + avg_wealth_sd, fill=Treatment),alpha=0.3) +
  xlab("Round")+
  ylab("Average wealth") +
  theme_bw() 

g.avg_coop = ggplot(data=plot.trends, aes(x=round,y=avg_coop_mean,group=Treatment)) +
  geom_line(aes(color=Treatment)) +
  geom_ribbon(aes(ymin = avg_coop_mean - avg_coop_sd, ymax = avg_coop_mean + avg_coop_sd, fill=Treatment),alpha=0.3) +
  xlab("Round")+
  scale_y_continuous(limits = c(0, 1)) +
  ylab("Proportion of cooperation") +
  theme_bw() 

g.avg_degree = ggplot(data=plot.trends, aes(x=round,y=avg_degree_mean,group=Treatment)) +
  geom_line(aes(color=Treatment)) +
  geom_ribbon(aes(ymin = avg_degree_mean - avg_degree_sd, ymax = avg_degree_mean + avg_degree_sd, fill=Treatment),alpha=0.3) +
  xlab("Round")+
  ylab("Average degree") +
  theme_bw() 
