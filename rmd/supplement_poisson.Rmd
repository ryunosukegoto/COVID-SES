...
# rmarkdown::render(paste(rootdir,"rmd/supplement_poisson.Rmd", sep="/"))
...
fit_linear <- glm(deaths_2021~pctPoverty*Series_Complete_Pop_Pct+PropMale+PropAbove65+no.college.diploma+native+
           asian+black+white+hispanic+state, data=d_l_rf, family = gaussian())

fit_poisson <- glm(deaths_2021~pctPoverty*Series_Complete_Pop_Pct+PropMale+PropAbove65+no.college.diploma+native+
           asian+black+white+hispanic+state, data=d_l_rf, family = poisson(link="log"))
```
...
fit_l <- cmest(
  ...
  yreg="poisson", mreg=rep(list("linear"),length(M)),
...
res_mediation_max_vax <- data.frame(
  ...
  "Effect" = 
    round(c(summary(fit_l)$effect.pe["Rte"],summary(fit_l)$effect.pe["Rcde"],summary(fit_l)$effect.pe["pe"]),2),
  "Lower CL" = 
    round(c(summary(fit_l)$effect.ci.low["Rte"],summary(fit_l)$effect.ci.low["Rcde"],summary(fit_l)$effect.ci.low["pe"]),2),
  "Upper CL" = 
    round(c(summary(fit_l)$effect.ci.high["Rte"],summary(fit_l)$effect.ci.high["Rcde"],summary(fit_l)$effect.ci.high["pe"]),2),
  "P-value" = 
    round(c(summary(fit_l)$effect.pval["Rte"],summary(fit_l)$effect.pval["Rcde"],summary(fit_l)$effect.pval["pe"]),3)
)
